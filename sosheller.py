...
def compileShellDex():
    ...
    licmd2.append(stCurrentPt + "\shellApplicationSourceCode\java\cn\yongye\\nativeshell\*.java")
...
def addLib(fp):
    stLib2Fp = os.path.join(stCurrentPt, "lib", "libyongyejiagu.so")

    apk = zipfile.ZipFile(fp, "a")
    apk.write(stLib2Fp, "lib/x86/libyongyejiagu.so")
    apk.close()
